...
def test_sample_from_buffers():
    ...
    experiences = np.array(
        [[-1, 0, 0, 0, 1], [1, 0.002, 0, 1, 0], [-1, 0.005, 1, 0, 0]]
    )
    ...
    data, labels = sample_from_buffers(
        demo_buffer, exp_buffer, batch_size, action_space_size
    )
    for data_member in data:
        ...
        for experience in experiences:
            data_in_experiences = (
                np.all(data_member == experience)
                if not data_in_experiences
                else data_in_experiences
            )
        ...
        for expert_state in demo_buffer_states:
            data_in_expert = (
                np.all(data_member[:2] == expert_state)
                if not data_in_expert
                else data_in_expert
            )
    ...
    assert np.all(
        labels
        == np.array([1] * (batch_size - len(experiences)) + [0] * len(experiences))
    )
