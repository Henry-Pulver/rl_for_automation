...
def value_iteration() -> np.array:
    ...
    while True:
        ...
        for state in STATE_SPACE:
            ...
            outcome_states = [get_new_state(state, action)
                              for action in possible_actions]
            rewards = [grid.get_reward(state) + value_fn[outcome_state]
...
def main():
    ...
    while True:
        possible_actions = playable_grid.find_possible_actions(playable_grid.agent_state)
        outcome_states = [get_new_state(playable_grid.agent_state, action)
                          for action in possible_actions]
        rewards = [playable_grid.get_reward(playable_grid.agent_state)
                   + value_function[outcome_state]
                   for outcome_state in outcome_states]
        best_actions = [action for action, reward in zip(possible_actions, rewards)
