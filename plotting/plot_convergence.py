...
def plot_data(
    ...
    for smooth in {False, plot_smoothed}:
        ...
        if len(plot.shape) > 1 and plot.shape[0] > 1:
            show_legend = True
        ...
        for count, theta in enumerate(plot):
            ...
            fig.add_trace(
                go.Scatter(
                    x=x,
                    y=theta,
                    line_color=PLOTLY_DEFAULT_COLOURS[count],
            ...
            if std_devs is not None:
                ...
                y_upper = list(
                    np.clip(
                        theta + std_devs[count], std_dev_limits[0], std_dev_limits[1]
                    )
                )
                y_lower = list(
                    np.clip(
    ...
    for entry in load_path.iterdir():
        ...
        # Plots
            ...
            plot_data(
                ...
                graph_labels,
                reward_smoothing_weight is not None and (entry.stem == "rewards" or entry.stem[-4:] == "loss"),
            )
...
def plot_demo_num_avgs(
    ...
    std_devs = (
        [var ** (1 / 2) for var in vars] if std_dev_plot_name is not None else None
    )
...
def main():
    ...
    #     )
    # base_path = Path(f"../reinforcement_learning/data/pretrained-PPO-clip/Breakout-ram-v4/13-05-2020/hyp-0.2/128-128-128-128")
    base_path = Path(f"../reinforcement_learning/data/PPO-clip/MountainCar-v0/15-05-2020/hyp-0.2/32-32")
    # base_path = Path(f"../reinforcement_learning/data/REINFORCE/Breakout-ram-v4/14-05-2020/hyp-REINFORCE/128-128-128-128")
    # base_path = Path(
    #     f"../imitation_learning/data/GAIL-clip/Acrobot-v1/09-05-2020/hyp-0.1-num_demos_100/32-32"
    # )
    # seed = 0
    # print(np.load(f"{base_path}/seed-1/params/param_names_array.npy"))

    for seed in range(2):
        graph_labels = {
            "Title": "",  # f"Atari Beam Rider clipped PPO smoothed rewards",
            # "x": "",
            "x": "Episode number",
            # "y": "",
            "y": "Score",
